...
-- FIX: Wrapped wallust require in pcall so hyprlock doesn't hard-crash if
-- wallust hasn't generated its theme file yet (e.g. on a fresh install).
local wallust_ok, wallust = pcall(require, "wallust/wallust-hyprland")
if not wallust_ok then
    -- Fallback colors if wallust theme is not yet generated
    wallust = {
        color11 = "rgba(255,255,255,1)",
        color12 = "rgba(200,200,255,1)",
        color13 = "rgba(255,255,255,1)",
    }
end
...
hl.config({
    ["input-field"] = {
        ...
        fade_on_empty = false,
        font_family = "JetBrainsMono Nerd Font",
        -- FIX: was ##ffffff99 (double hash) — Pango foreground color requires a single #

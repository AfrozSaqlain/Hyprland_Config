...
# --- MODIFICATION START ---
# Get all monitor names into an array
mapfile -t all_monitors < <(hyprctl monitors -j | jq -r '.[].name')
# --- MODIFICATION END ---
...
# Monitor details (for Rofi sizing, based on the focused monitor where Rofi appears)
...
modify_startup_config() {
  ...
  if [[ "$selected_file" =~ \.(mp4|mkv|mov|webm)$ ]]; then
    ...
    sed -i '/^\s*exec-once\s*=\s*awww\s*restore\s*$/s/^/\#/' "$startup_config"
    ...
    sed -i '/^\s*#\s*exec-once\s*=\s*awww\s*restore\s*$/s/^\s*#\s*//;' "$startup_config"
...
apply_image_wallpaper() {
  ...
  # --- MODIFICATION START ---
  # Loop through all monitors and apply the wallpaper to each.
  for monitor in "${all_monitors[@]}"; do
    awww img -o "$monitor" "$image_path" $AWWW_PARAMS
  done
  # --- MODIFICATION END ---
...
apply_video_wallpaper() {
  ...
  # Apply video wallpaper using mpvpaper. The '*' targets all monitors by default.
